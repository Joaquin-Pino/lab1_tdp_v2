...
Pieza::Pieza(const Pieza& otra)
    ...
    if (otra.geometria) {
        // Copiar celda por celda el arreglo aplanado de tamaño ancho*alto.
        // No podemos copiar solo el puntero porque ambos objetos liberarían el mismo bloque.
...
Pieza& Pieza::operator=(const Pieza& otra) {
    ...
    // Liberar la geometría actual antes de sobreescribir las dimensiones,
    // ya que ancho y alto cambian con la asignación.
    delete[] geometria;
...
short Pieza::getAncho() const { return ancho; }
short Pieza::getAlto() const { return alto; }
int Pieza::getColor() const { return color; }
coordenada Pieza::getPosInicial() const { return posInicial; }
bool* Pieza::getGeometria() const { return geometria; }

bool Pieza::getCelda(int x, int y) const {
    if (x < 0 || x >= ancho || y < 0 || y >= alto)
        throw std::out_of_range("Coordenadas fuera de los limites de la pieza");
    // Acceso fila-mayor: la fila y comienza en el índice y*ancho del arreglo.
